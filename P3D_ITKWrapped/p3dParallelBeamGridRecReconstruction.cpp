...
int _p3dParallelBeamGridRecReconstruction_32 (				// breadth-first version of the algorithm
	 float* in_im,
	 float* out_im,
	 ...
	 )
{
	...
	typedef ReconstructionFilterType::OutputImageType InternalImageType;
        ReconstructionFilterType::Pointer filter = ReconstructionFilterType::New();
	
	filter->SetInput( importFilter->GetOutput() );

	filter->SetRDirection( dimx );
	filter->SetZDirection( 1 );
	filter->SetAlphaDirection( dimy );
	filter->SetZeroPadding( 2 );
	filter->SetOverSampling( 2 );
	filter->SetCutoff( 1.0 );
	filter->SetRadialSplineOrder( 3 );
	filter->SetAlphaRange( 180 );
