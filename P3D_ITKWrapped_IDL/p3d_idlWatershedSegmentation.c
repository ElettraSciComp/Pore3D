...
IDL_VPTR p3d_idlWatershedSegmentation(int argc, IDL_VPTR argv[], char* argk)
{ 
	...
	{	
		...
		if (idl_in_rev->type == IDL_TYP_BYTE)			
		{
			...
			// Call Pore3D:
			err_code = p3dWatershedSegmentation2D_8 ( 
				in_rev8,
				out_rev8,
		...
		else if (idl_in_rev->type == IDL_TYP_UINT)		
		{
			...
			// Call Pore3D:
			err_code = p3dWatershedSegmentation2D_16 ( 
				in_rev16,
				out_rev8,
				(unsigned int) idl_in_rev->value.arr->dim[0],
				(unsigned int) idl_in_rev->value.arr->dim[1],
	...
	else if ( idl_in_rev->value.arr->n_dim == 3 )
	{
		...
		if (idl_in_rev->type == IDL_TYP_BYTE)			
		{
			...
			// Call Pore3D:
			err_code = p3dWatershedSegmentation3D_8 ( 
				in_rev8,
				out_rev8,
				(unsigned int) idl_in_rev->value.arr->dim[0],
				(unsigned int) idl_in_rev->value.arr->dim[1],
		...
		else if (idl_in_rev->type == IDL_TYP_UINT)		
		{
			...
			err_code = p3dWatershedSegmentation3D_16 ( 
				in_rev16,
				out_rev8,
				(unsigned int) idl_in_rev->value.arr->dim[0],
				(unsigned int) idl_in_rev->value.arr->dim[1],
