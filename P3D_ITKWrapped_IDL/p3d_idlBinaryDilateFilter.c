...
IDL_VPTR p3d_idlBinaryDilateFilter(int argc, IDL_VPTR argv[], char* argk)
{ 
	typedef struct {  
	...
	{	
		...
		if (idl_in_rev->type == IDL_TYP_BYTE)			
		{
			...
			err_code = p3dBinaryDilateFilter2D ( 
				in_rev8,
				out_rev8,
	...
	else if ( idl_in_rev->value.arr->n_dim == 3 )
	{
		...
		if (idl_in_rev->type == IDL_TYP_BYTE)			
		{
			...
			// Call Pore3D:
			err_code = p3dBinaryDilateFilter3D ( 
				in_rev8,
				out_rev8,
				(unsigned int) idl_in_rev->value.arr->dim[0],
				(unsigned int) idl_in_rev->value.arr->dim[1],
