...
IDL_VPTR p3d_idlMinMaxCurvatureFlowFilter(int argc, IDL_VPTR argv[], char* argk)
{ 
	typedef struct {  
	...
	{	
		...
		if (idl_in_rev->type == IDL_TYP_BYTE)			
		{
			...
			err_code = p3dMinMaxCurvatureFlowFilter2D_8 ( 
				in_rev8,
				out_rev8,
		...
		else if (idl_in_rev->type == IDL_TYP_UINT)		
		{
			...
			// Call Pore3D without the mask:
			err_code = p3dMinMaxCurvatureFlowFilter2D_16 ( 
				in_rev16,
				out_rev16,
				(int) idl_in_rev->value.arr->dim[0],
				(int) idl_in_rev->value.arr->dim[1],
	...
	else if ( idl_in_rev->value.arr->n_dim == 3 )
	{
		...
		if (idl_in_rev->type == IDL_TYP_BYTE)			
		{
			...
			// Call Pore3D without the mask:
			err_code = p3dMinMaxCurvatureFlowFilter3D_8 ( 
				in_rev8,
				out_rev8,
				(int) idl_in_rev->value.arr->dim[0],
				(int) idl_in_rev->value.arr->dim[1],
		...
		else if (idl_in_rev->type == IDL_TYP_UINT)		
		{
			...
			// Call Pore3D without the mask:
			err_code = p3dMinMaxCurvatureFlowFilter3D_16 ( 
				in_rev16,
				out_rev16,
				(int) idl_in_rev->value.arr->dim[0],
				(int) idl_in_rev->value.arr->dim[1],
