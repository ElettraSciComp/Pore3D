...
IDL_VPTR p3d_idlMinVolumeFilter(int argc, IDL_VPTR argv[], char* argk)
{ 
	typedef struct {  
	...
	static IDL_KW_PAR kw_pars[] = {  
		...
		{ "MIN_VOL", IDL_TYP_LONG, 1, 0, (int*) IDL_KW_OFFSETOF(mv_there), (char*) IDL_KW_OFFSETOF(min_vol) },  		
    	{ NULL }  
	 };  
	...
	int conn2D = CONN4;		// default
	int conn3D = CONN6;		// default

	unsigned int min_vol = 5;			// default	
	...
	// Get the MIN_VOL input argument:
	if (kw.mv_there)
	{
		// Check values:
		if ( kw.min_vol < 0 )
			_p3d_idlPrintNamedError("MIN_VOL must be greater than 0.");

		// Get values:
		min_vol = (unsigned int) kw.min_vol;
