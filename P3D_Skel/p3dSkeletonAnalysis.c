...
int _p3dSkeletonAnalysis_Tortousity(
    ...
    float* min_ct_array = NULL;
    float* max_ct_array = NULL;
    int* coord_array = NULL;

    float max_width, dist1, dist2;

    int cc_array_numel;
    float cc_length, min, max;
    int max_i, max_j, max_k;
    unsigned int* cc_array = NULL;
    ...
    // Allocate memory:	
    P3D_TRY(poresplusend_im = (unsigned char*) calloc(dimx * dimy * dimz, sizeof (unsigned char)));
    P3D_TRY(tmp_im = (unsigned short*) calloc(dimx * dimy * dimz, sizeof (unsigned short)));
    P3D_TRY(tmp_im2 = (unsigned short*) calloc(dimx * dimy * dimz, sizeof (unsigned short)));
    ...
    if (cc_array_numel != 0) {
        ...
        P3D_TRY(tort_matrix = (float*) calloc(cc_array_numel * cc_array_numel, sizeof (float)));
        P3D_TRY(tort_array = (float*) calloc(cc_array_numel * 4, sizeof (float)));
        P3D_TRY(coord_array = (int*) calloc(cc_array_numel, sizeof (int)));
        ...
        for (ct = 0; ct < cc_array_numel; ct++) {
            ...
            tort_array[ I2(ct, 0, cc_array_numel) ] = cen_i;
            tort_array[ I2(ct, 1, cc_array_numel) ] = cen_j;
            tort_array[ I2(ct, 2, cc_array_numel) ] = cen_k;
                        ...
                        if ((i >= 0) && (j >= 0) && (k >= 0) &&
                            ...
                            if (tmp_im2[ I(i, j, k, dimx, dimy) ] == USHRT_MAX) {
                                ...
                                if (_countNeighbors(tmp_im2, dimx, dimy, dimz, i, j, k,
                                    ...
                                    if (abs(cc_length - 0.0) < 1E-4)
                                        cc_length = 1.0;

                                    // Now add cc_length to output matrix:		
                                    if ((other_lbl != -3) && (ct != other_lbl)) {
                                        if (abs(tort_matrix[ I2(ct, other_lbl, cc_array_numel) ] - 0.0) < 1E-4) {
                                            tort_matrix[ I2(ct, other_lbl, cc_array_numel) ] = cc_length;
        ...
        for (i = 0; i < cc_array_numel; i++) {
            for (j = 0; j < cc_array_numel; j++) {
                if ((j > i) && (!abs(tort_matrix[ I2(i, j, cc_array_numel) ] - 0.0) < 1E-4)) {
        ...
        // Allocate memory:
        P3D_TRY(min_ct_array = (float*) calloc(num_nodes, sizeof (float)));
        P3D_TRY(max_ct_array = (float*) calloc(num_nodes, sizeof (float)));

        P3D_TRY(out_stats->Tort_X = (double*) calloc(out_stats->Tort_Counter, sizeof (double)));
        P3D_TRY(out_stats->Tort_Y = (double*) calloc(out_stats->Tort_Counter, sizeof (double)));
        P3D_TRY(out_stats->Tort_Z = (double*) calloc(out_stats->Tort_Counter, sizeof (double)));
...
int p3dSkeletonAnalysis(
   ...
   /* char* auth_code;
    ...
    //auth_code = authenticate("p3dSkeletonAnalysis");
