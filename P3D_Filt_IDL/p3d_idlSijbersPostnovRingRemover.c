...
IDL_VPTR p3d_idlSijbersPostnovRingRemover(int argc, IDL_VPTR argv[], char* argk) {
    ...
    /*if (argv[1]->type == IDL_TYP_BYTE)
        thresh = (double) argv[1]->value.c;
    else if (argv[1]->type == IDL_TYP_INT)
        thresh = (double) argv[1]->value.i;
    else if (argv[1]->type == IDL_TYP_UINT)
        thresh = (double) argv[1]->value.ui;
    else if (argv[1]->type == IDL_TYP_LONG)
        thresh = (double) argv[1]->value.l;
    else if (argv[1]->type == IDL_TYP_ULONG)
        thresh = (double) argv[1]->value.ul;
    else if (argv[1]->type == IDL_TYP_LONG64)
        thresh = (double) argv[1]->value.l64;
    else if (argv[1]->type == IDL_TYP_ULONG64)
        thresh = (double) argv[1]->value.ul64;
    else if (argv[1]->type == IDL_TYP_FLOAT)
        thresh = (double) argv[1]->value.f;
    else if (argv[1]->type == IDL_TYP_DOUBLE)
        thresh = (double) argv[1]->value.d;
    else
        _p3d_idlPrintNamedError("Input argument THRESH must be of type DOUBLE.");*/
